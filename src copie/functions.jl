...
function calculate_max_divisions(cell::Cell   )#coord::Tuple{Int64, Int64})
    println(round(Int64, sqrt((cell.coordinates[1]^2 + cell.coordinates[2]^2))))
    # Exemple simple : plus la cellule est éloignée de l'origine, plus elle peut se diviser.
    return round(Int64, sqrt((cell.coordinates[1]^2 + cell.coordinates[2]^2)))
end
...
function run_simulation(initial_cells::CellSetByCoordinates, num_steps::Int64, grid_size::Tuple{Int64, Int64},  cell_type_sequence::Vector{Int64}; xml_file::String = "cellTypesChange.xml", max_div_sequence::Union{Vector{Int64}, Nothing} = nothing)
    history = [deepcopy(initial_cells)]
    current_cells = CellSetByCoordinates(Dict{Tuple{Int64, Int64}, Cell}())
    current_cells.cells = deepcopy(initial_cells.cells)
    cell_data=load_cell_data(xml_file, cell_types_sequence)
    
    if max_div_sequence === nothing && cell_data == nothing
        max_cell_divisions === nothing
    elseif max_div_sequence === nothing
        max_cell_divisions = create_max_cell_divisions_dict(cell_data, current_cells)
    else
