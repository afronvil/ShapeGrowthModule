...
# --- La fonction inverse : undo_proliferate! ---
"""
    undo_proliferate!(model::CellModel{Dim}, next_cells::Dict{NTuple{Dim, Int64}, Cell{Dim}}, 
                     parent_coords::NTuple{Dim, Int64}, new_cell_coords::NTuple{Dim, Int64}) where Dim

Annule les effets d'une prolifération réussie par `try_proliferate!`.

ATTENTION : Cette fonction doit être appelée seulement si `try_proliferate!` a précédemment
retourné `true` pour les `parent_coords` et `new_cell_coords` données.
Elle ne gère pas les cas où la prolifération n'a pas eu lieu ou a échoué.

Arguments:
- `model`: Le modèle de cellules (non strictement nécessaire pour l'annulation elle-même, mais gardé pour cohérence).
- `next_cells`: Le dictionnaire des cellules qui a été mis à jour par `try_proliferate!`.
- `parent_coords`: Les coordonnées de la cellule parente.
- `new_cell_coords`: Les coordonnées de la nouvelle cellule enfant créée par prolifération.

Retourne:
- `true` si l'annulation a été effectuée avec succès, `false` sinon (par exemple, si les cellules n'étaient pas trouvées).
"""
function undo_proliferate!(model::CellModel{Dim}, next_cells::Dict{NTuple{Dim, Int64}, Cell{Dim}}, 
                           parent_coords::NTuple{Dim, Int64}, new_cell_coords::NTuple{Dim, Int64}) where Dim
    
    # 1. Vérifier si la cellule enfant existe et la supprimer
    if haskey(next_cells, new_cell_coords)
        delete!(next_cells, new_cell_coords)
    else
        # Si la nouvelle cellule n'est pas là, c'est une anomalie, ou undo_proliferate! a été appelé à tort.
        @warn "undo_proliferate!: New cell at $new_cell_coords not found in next_cells. Skipping undo."
        return false
    end

    # 2. Restaurer l'état de la cellule parente
    if haskey(next_cells, parent_coords)
        parent_cell_after_prolif = next_cells[parent_coords]

        # Hypothèse: nous pouvons décrémenter nbdiv et changer le flag.
        # Idéalement, on aurait sauvegardé l'état PRE-prolifération.
        # Ici, nous déduisons l'état original.
        restored_parent_cell = Cell{Dim}(
            parent_cell_after_prolif.coordinates,
            parent_cell_after_prolif.id,
            parent_cell_after_prolif.cell_type,
            parent_cell_after_prolif.initial_type,
            # Supposons que last_division_type n'est pas important à restaurer, 
            # ou qu'il était le 'cell_type' avant la division
            parent_cell_after_prolif.initial_type, 
            parent_cell_after_prolif.nbdiv - 1, # Décrémenter nbdiv
            parent_cell_after_prolif.initial_nbdiv,
            parent_cell_after_prolif.is_alive,
            false, # has_proliferated_this_step redevient false
            parent_cell_after_prolif.current_type_index_in_sequence
        )
        next_cells[parent_coords] = restored_parent_cell
        return true
    else
        @warn "undo_proliferate!: Parent cell at $parent_coords not found in next_cells. Skipping undo."
        return false
    end
end
...
#= function attempt_proliferation!(model::CellModel{Dim} ,next_cells::Dict{NTuple{Dim, Int64}, Cell{Dim}}, cell::Cell{Dim}, cell_directions_int::Vector{Int64}, directions::Vector{NTuple{Dim, Int64}}, max_cell_division::Real) where Dim
    ...
    if !found_close_stromal_cell
        ...
        new_stromal_cell = StromalCell{Dim}(
                cell.coordinates,
                cell.timer,
                cell.cell_type,
                cell.cell_type,
...
function attempt_proliferation!(
    ...
    if !found_close_stromal_cell && !isnothing(model.stromal_cells) && !isempty(model.stromal_cells)
        ...
        if isnothing(model.stromal_cells)
            model.stromal_cells = Dict{NTuple{Dim, Int64}, StromalCell{Dim}}()
        end
