            ...
            for cell_type in findall("cellType", genome)
                ...
                if type_id in cell_types
                    try
                        ...
                        dir_value = parse(Int64, cell_type["nbDir"])
                        ...
                        #println(directions)
                        #cell_type_colors[type_id] = RGB(color0, color1, color2)
                        #println(cell_type_colors[type_id])
                        cell_data[type_id] = Dict("directions" => directions, "color" => RGB(color0, color1, color2))
                        #println(cell_data[type_id])
                    catch e
                        ...
                        cell_data[type_id] = Dict("directions" => Int64[],"color" => RGB(0.0, 0.0, 0.0))
                    end
                end
            end
        end
