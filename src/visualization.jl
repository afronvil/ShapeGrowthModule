...
block_size_rows = 5
block_size_cols = 5
function visualization(model::ShapeGrowthModule.CellModel{Dim}) where Dim 
    ...
    if Dim == 2 
        ...
        ShapeGrowthModule.visualize_final_state_2D(model, final_image_filename_2D, block_size_rows, block_size_cols)
        ...
        ShapeGrowthModule.visualize_history_animation_2D(model, animation_filename_2D, block_size_rows, block_size_cols, 10) 
        ...
        ShapeGrowthModule.visualize_3D_cells(model, Dim, cell_type_sequence) 
        ...
        ShapeGrowthModule.visualize_history_3D_plotly_frames(model, Dim, output_dir_3D)
